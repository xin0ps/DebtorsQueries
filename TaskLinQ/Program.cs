...
class Program
{
    ...
    public class DebtorContext:DbContext
    {
        ...
        {
            string constr = "Data Source=LAPTOP-JLBUNNBV\\SQLEXPRESS;Initial Catalog=Debtors;Integrated Security=True;Connect Timeout=30;Encrypt=True;Trust Server Certificate=True;Application Intent=ReadWrite;Multi Subnet Failover=False";
        ...
        {
            modelBuilder.Entity<Debtor>().HasKey(x => x.Id);
    ...
    static void Main()
    {
        ...
        //      using (var _context = new DebtorContext())
        //      {
        ...
        var _context = new DebtorContext();

        //        2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag


        //var task2 = _context.Debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
        ...
        //var task3 = _context.Debtors.Where(d => (2024 - d.BirthDay.Year)>26&& (2024 - d.BirthDay.Year) <36).ToList();
        ...
        //var task4 = _context.Debtors.Where(d => d.Debt<5000).ToList();
        ...
        //   var task5 = _context.Debtors
        //.Where(d => d.FullName.Length > 18).ToList().Where(d => d.Phone.Count(c => c == '7') >= 2).ToList();
        ...
        //var task6 = _context.Debtors.Where(d => d.BirthDay.Month == 1 || d.BirthDay.Month == 12 || d.BirthDay.Month == 2).ToList();

        //foreach (var debtor in task6)
        //{
        //    Console.WriteLine(debtor);
        //}
        ...
        //var average = _context.Debtors
        //    .Where(d => d.Debt > 0) 
        //    .Select(d => d.Debt) 
        //    .Average();

        //var task7 = _context.Debtors.Where(d => d.Debt>average).OrderBy(d => d.FullName) 
        //  .ThenByDescending(d => d.Debt).ToList();
        ...
        //var task8 = _context.Debtors.ToList().Where(d => d.Phone.Contains('8')).ToList();
        //Console.WriteLine("FullName \tAge\t Debt  ");
        ...
        // var task11 = _context.Debtors
        //.ToList()
        //.Where(d => d.FullName.GroupBy(c => c).Any(group => group.Count() >= 3))
        //.OrderBy(d => d.FullName).ThenBy(d=>d.FullName)
        //.ToList();
        ...
        var year = _context.Debtors.GroupBy(d => d.BirthDay.Year).Select(s => new { year = s.Key, count = s.Count() }).OrderByDescending(d => d.count).FirstOrDefault();

      
            Console.WriteLine($"{year.year} - {year.count}");
        //14) Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
        var top5Debtors = _context.Debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
        ...
        var totalDebt = _context.Debtors.Sum(d => d.Debt);
        Console.WriteLine($"Umumi borc: {totalDebt}");

        //16) 2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
        var ww2Veterans = _context.Debtors.Where(d => d.BirthDay.Year<1946).ToList();
        foreach (var veteran in ww2Veterans)
        {
            Console.WriteLine($"{veteran.FullName}");
        }

        //18) Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
        var uniqueNumberDebtors = _context.Debtors.Where(d => d.Phone.Distinct().Count() == d.Phone.Length).ToList();
        foreach (var debtor in uniqueNumberDebtors)
        {
            Console.WriteLine($"{debtor.FullName} - {debtor.Debt}");
        }
        ...
        var paidOffDebtors = _context.Debtors.Where(d => d.Debt == 0 || d.BirthDay.AddYears(today.Year - d.BirthDay.Year).AddDays(1) < today).ToList();
        foreach (var debtor in paidOffDebtors)
        {
            Console.WriteLine($"{debtor.FullName}");
        }

        //20) Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq 
        var smileDebtors = _context.Debtors.Where(d => d.FullName.Contains("smile", StringComparison.OrdinalIgnoreCase)).ToList();
